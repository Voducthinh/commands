module.exports.config = {
  ...
  version: "1.0.0",
  hasPermssion: 0,
  credits: "NTKhang Fix be hoàng anh",
...
module.exports.handleReply = async function({ api, args, event, handleReply}) {
 ...
 switch(handleReply.type) {
   ...
   break;}
    case "calladmin": {
      api.sendMessage({ body: `📌Phản hồi từ Admin Bot ${name} :\n${event.body}\n---------\n💬Reply ( trả lời ) tin nhắn này để tiếp tục gửi báo cáo cho Admin Bot!`, mentions: [{tag: name, id : event.senderID}]}, handleReply.id, (e, data) => global.client.handleReply.push({
  name: this.config.name,
  author: event.senderID,
  messageID: data.messageID,
